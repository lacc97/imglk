...
void glk_main(void) {
  ...
  statuswin = glk_window_open(mainwin, winmethod_Above | winmethod_Fixed, 3,
                              wintype_TextGrid, 0);
...
static void verb_help(void) {
  ...
  glk_put_string("MOVE: A verb which prints some text, and also changes the "
                 "status line display.\n");
  glk_put_string("QUOTE: A verb which displays a block quote in a temporary "
                 "third window.\n");
  glk_put_string("SCRIPT: Turn on transcripting, so that output will be echoed "
                 "to a text file.\n");
...
#define NUMWORDS (13)
  static char *wordcaplist[NUMWORDS] = {"Ga", "Bo", "Wa", "Mu", "Bi",
                                        "Fo", "Za", "Mo", "Ra", "Po",
                                        "Ha", "Ni", "Na"};
...
static void verb_quote(void) {
  ...
  if (!quotewin) {
    ...
    quotewin = glk_window_open(mainwin, winmethod_Above | winmethod_Fixed, 5,
...
static void verb_script(void) {
  ...
  glk_put_string_stream(scriptstr, "This is the beginning of a transcript.\n");
}
...
static void verb_unscript(void) {
  ...
  /* Close the file. */
  glk_put_string_stream(scriptstr, "This is the end of a transcript.\n\n");
